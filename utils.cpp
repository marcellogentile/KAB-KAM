...
void getExtIP()
{
  ...
  if (hclient.connect(extIpHost, 80))
  {
    ...
    uint32_t startAttemptTime = millis();
    while (!hclient.available() && millis() - startAttemptTime < 5000)
      delay(500);
    if (hclient.available())
    {
      String newExtIp = "";
      while (hclient.available())
        newExtIp += hclient.readStringUntil('\r');
      if (newExtIp.length())
      {
        if (strstr(newExtIp.c_str(), "200 OK") != NULL)
        {
          ...
          LOG_ERR("Bad request response");
      }
      else
        LOG_ERR("External IP not retrieved");
    }
    else
      LOG_ERR("External IP no response");
    hclient.stop();
  }
  else
    LOG_ERR("ExtIP connection failed");
  ...
  if (xSemaphoreTake(logMutex, logWait / portTICK_PERIOD_MS) == pdTRUE)
  {
    ...
    if (logMode)
    {
      if (log_remote_fp != NULL)
      {
        ...
        ramLogStore(msgLen); // store in ram instead
    }
...
void formatHex(const char *inData, size_t inLen)
{
  ...
  for (int i = 0; i < inLen; i++)
    sprintf(formatted + (i * 3), "%02x ", inData[i]);
